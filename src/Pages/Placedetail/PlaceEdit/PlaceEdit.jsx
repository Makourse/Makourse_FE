...
const simplifyCategory = (category) => {
  ...
  // 카테고리 문자열을 분리 (>, 쉼표로 구분된 경우 모두 처리)
  const categories = category.split(/[,>]/).map(cat => cat.trim());
  
  // 첫 번째로 매칭되는 간소화된 카테고리 반환
  for (const [simplifiedCat, keywords] of Object.entries(categoryMap)) {
...
const PlaceEdit = () => {
  ...
  const handleSuggestionClick = (suggestion) => {
    const { latitude, longitude, name, address, category } = suggestion;
    ...
    try {
      ...
      if (response && response.items && Array.isArray(response.items)) {
        const formattedResults = response.items.map(item => ({
          ...
          category: simplifyCategory(item.category), // 카테고리 간소화 적용
